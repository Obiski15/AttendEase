...
* **`feat`**: A new feature (e.g., `feat(ui): add lecturer active session timer`)
* **`fix`**: A bug fix (e.g., `fix(scan): prevent scanner crash on empty string`)
* **`refactor`**: Code changes that neither fix bugs nor add features (e.g., `refactor(common): extract shared confirm dialog`)
* **`style`**: Formatting, whitespace, indentation corrections.
